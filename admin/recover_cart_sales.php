...
/*
 History
 - $Id: recover_cart_sales.php,v 1.6 2005/06/22 06:10:35 lane Exp $ Recover Cart Sales Tool v2.11
 - Copyright (c) 2003-2005 JM Ivler / Ideas From the Deep / OSCommerce  Released under the GNU General Public License
 - Based on an original release of unsold carts by: JM Ivler;  That was modifed by Aalst (aalst@aalst.com) until v1.7 of stats_unsold_carts.php;   Then, the report was turned into a sales tool (recover_cart_sales.php) by  JM Ivler based on the scart.php program that was written off the Oct 8 unsold carts code release.
 - Modifed by Aalst (recover_cart_sales.php,v 1.2 ... 1.36)  aalst@aalst.com
 - Modifed by willross (recover_cart_sales.php,v 1.4)  reply@qwest.net  - don't forget to flush the 'scart' db table every so often
 - Modified by Lane Roathe (recover_cart_sales.php,v 1.4d .. v2.11)  lane@ifd.com   www.osc-modsquad.com / www.ifd.com
 - Modified v4.00.00a 2012/04/23 testuser -> iChoze -> Zen Cart v1.5

 Modified v4.2.00 2026-05-09 OldNGrey -> international dates DATE_FORMAT_INTL (update lang.recover_cart.php in english/extra_definitions as well), customer ID appended to customer name for better identification, maintain sort order by date added,  reformatted code
 BMH 2026-05-09 - updated lang file with new defines and updated HEADING_TITLE define to avoid conflict with other pages, change buttom images to css
*/
require('includes/application_top.php');

const RECOVER_CART_SALES_VERSION = '4.2.00'; // BMH 2026-05-09 - created and updated version number
...
function zen_cart_date_short($raw_date)
{
    ...
    if (@date('Y', mktime(0, 0, 0, $month, $day, $year)) == $year) {
        // BMH return date(DATE_FORMAT, mktime(0, 0, 0, $month, $day, $year));
        return date(DATE_FORMAT_INTL, mktime(0, 0, 0, $month, $day, $year)); // BMH
    } else {
        // BMH return preg_replace('/2037$/', $year, date(DATE_FORMAT, mktime(0, 0, 0, $month, $day, 2037))); /*lat9-c*/  // BMH
        return preg_replace('/2037$/', $year, date(DATE_FORMAT_INTL, mktime(0, 0, 0, $month, $day, 2037))); /*lat9-c*/
    }
}
...
<head>
    ...
    <?php if ($editor_handler != '') {
        $PHP_SELF_save = $PHP_SELF;
        $PHP_SELF = 'mail.php';
        include($editor_handler);
        $PHP_SELF = $PHP_SELF_save;
    } ?>
</head>
        ...
        <tr>
            ...
            <td width="100%" valign="top">
                <table border="0" width="100%" cellspacing="0" cellpadding="0">
                            ...
                            <table border="0" width="100%" cellspacing="2" cellpadding="2">
                                <tr>
                                    <td class="pageHeading">
                                        <?php echo RECOVER_CART_HEADING_TITLE . '  Version: ' . RECOVER_CART_SALES_VERSION; ?>
                                    </td>
                    ...
                    <tr>
                                                <td colspan='6' align='right'><a href=" . zen_href_link(FILENAME_RECOVER_CART_SALES, "action=delete&customer_id=" . $cid . "&tdate=" . $tdate) . ' 211 class="button" role="button" >' . IMAGE_RECOVER_CART_DELETE . '</a></td>' . "
                    </tr>\n";
                                                    echo $cline;
                                                }
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="8" align="right" class="dataTableContent">
                                            <?php echo '<b>' . TABLE_CART_TOTAL . '</b>' . $currencies->format($tprice); ?>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="6" align="right"><a
                                                href="<?php echo 'ZZ' . zen_href_link(FILENAME_RECOVER_CART_SALES, "action=delete&customer_id=" . ($cid ?? '-1') . "&tdate=" . $tdate); ?>"
                                                <?php echo '314 class="button" role="button" >' . IMAGE_RECOVER_CART_DELETE; ?></a></td>
                                    </tr>

                                    <tr>
                                        <td colspan="6" align="center"><a
                                                href="<?php echo zen_href_link(FILENAME_RECOVER_CART_SALES); ?>"><?php echo TEXT_RETURN; ?></a>
                                        </td>
                                    </tr>
                                </table>
                            </td>
                                ...
                                <table border="0" width="100%" cellspacing="2" cellpadding="2">
                                    <tr class="dataTableHeadingRow">
                                        <td class="dataTableHeadingContent" align="left" colspan="2" width="10%">
                                            <?php echo TABLE_HEADING_CONTACT; ?>
                                        </td>
                                        <td class="dataTableHeadingContent" align="left" colspan="1" width="15%">
                        ...
                        <tr>
                            <td colspan="8" align="right"><a
                                    href="<?php echo zen_href_link(FILENAME_RECOVER_CART_SALES, 'action=setcontacted&customer_id=' . $curcus . '&tdate=' . $tdate); ?>"><?php echo TEXT_SET_CONTACTED; ?></a>&nbsp;
                                <a href="<?php echo zen_href_link(FILENAME_RECOVER_CART_SALES, 'action=delete&customer_id=' . $curcus . '&tdate=' . $tdate); ?>"
                                    <?php echo ' class="btn btn-primary" role="button" >' . IMAGE_RECOVER_CART_DELETE; ?> </a>
                            </td>
                        </tr>
                    <?php }
                                            ...
                                            $customer = $basket->fields['customers_firstname'] . " " . $basket->fields['customers_lastname'] . ' (#' . $basket->fields['customers_id'] . ')'; // BMH added customer ID to name for
                                            $status = "";
                                        ...
                                        WHERE (customers_id = ' . (int) $curcus . '
                                            ...
                                            if ($orec->RecordCount() > 0) {
                                                ...
                                                if (RCS_CHECK_REPEAT == 'true') {
                                                    ...
                                                    // BMH $customer = '<font color=' . RCS_CURCUST_COLOR . '><b>' . $customer . '</b></font>';
                                                    $customer = '<font color=' . RCS_CURCUST_COLOR . '><b>' . $customer . ' ' . $basket->fields['customers_id'] . '</b></font>';  // BMH DEBUG 
                    ...
                    <tr bgcolor="<?php echo $fcolor; ?>">
                        <td class="dataTableContent" align="center" width="1%">
                            <?php echo zen_draw_checkbox_field("custid[]", $curcus, RCS_AUTO_CHECK == "true" ? $checked : 0); ?>
                        </td>
                        <td class="dataTableContent" align="left" width="9%">
                            <b><?php echo ($sentdate != "" ? zen_cart_date_short($sentdate) : TEXT_NOT_CONTACTED); ?></b>
                        </td>
                        <td class="dataTableContent" align="left" width="15%">
                            <?php echo zen_cart_date_short($basket->fields['customers_basket_date_added']); ?>
                        </td>
                        <td class="dataTableContent" align="left" width="30%"><a
                                href="<?php echo zen_href_link(FILENAME_CUSTOMERS, "search=" . $basket->fields['customers_lastname']); ?>"
                                target="_blank"><?php echo $customer; ?></a><?php echo $status; ?></td>
                        <td class="dataTableContent" align="left" colspan="2" width="30%"><a
                                href="<?php echo zen_href_link("mail.php", "selected_box=tools&customer=" . $basket->fields['customers_email_address']); ?>"
                                target="_blank"><?php echo $basket->fields['customers_email_address']; ?></a></td>
                        <td class="dataTableContent" align="left" colspan="2" width="15%">
                            <?php echo $basket->fields['customers_telephone']; ?>
                        </td>
                    </tr>
                    <?php
                                        }
                                        ...
                                        if (($basket->fields['customers_basket_quantity'] > 0) && zen_products_id_valid($basket->fields['products_id'])) {
                    ...
                    <tr class="dataTableRow">
                        ...
                        <td class="dataTableContent" align="left" vAlign="top" width="15%"><?php echo $products->fields['model']; ?>
                        </td>
                        <td class="dataTableContent" align="left" vAlign="top" colspan="2" width="55%"><a
                                href="<?php echo zen_href_link($zc_products->get_admin_handler($products->fields['products_type']), "page=" . ($_GET['page'] ?? '') . "&product_type=" . $products->fields['products_type'] . "&cPath=" . zen_get_product_path((int) $basket->fields['products_id']) . "&pID=" . (int) $basket->fields['products_id'] . "&action=new_product"); ?>"
                                target="_blank"><b><?php echo $products->fields['name']; ?></b></a><?php echo $prodAttribs; ?></td>
                        <td class="dataTableContent" align="center" vAlign="top" width="5%">
                            <?php echo $basket->fields['customers_basket_quantity']; ?>
                        </td>
                        <td class="dataTableContent" align="right" vAlign="top" width="5%">
                            <?php echo $currencies->format($sprice); ?>
                        </td>
                        <td class="dataTableContent" align="right" vAlign="top" width="10%">
                            <?php echo $currencies->format($basket->fields['customers_basket_quantity'] * $sprice); ?>
                        </td>
                    </tr>
                    <?php
                                        }
                ...
                <tr>
                    <td colspan='8' align='right'><a
                            href="<?php echo zen_href_link(FILENAME_RECOVER_CART_SALES, 'action=setcontacted&customer_id=' . $curcus . '&tdate=' . $tdate); ?>"><?php echo TEXT_SET_CONTACTED; ?></a>&nbsp;<a
                            href="<?php echo zen_href_link(FILENAME_RECOVER_CART_SALES, 'action=delete&customer_id=' . $curcus . '&tdate=' . $tdate); ?>"
                            <?php echo ' class="btn btn-primary" role="button" >' . IMAGE_RECOVER_CART_DELETE; ?></a>
                    </td>
                </tr>
            <?php } ?>
            <tr>
                <td class='dataTableContent' align='right' colspan='8'>
                    <hr align=right><?php echo '<b>' . TABLE_GRAND_TOTAL . '</b>' . $currencies->format($totalAll); ?>
                </td>
            </tr>
        </table>
        </td>
            ...
            <td>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td><?php echo zen_draw_input_field('from', (isset($_POST['from']) ? $_POST['from'] : STORE_OWNER . ' <' . STORE_OWNER_EMAIL_ADDRESS . '>'), 'size="60" disabled="disabled"'); ?>
                        </td>
                    </tr>
                    <tr>
                        <td valign="top" class="main"><?php echo TEXT_SUBJECT; ?></td>
                        <td><?php echo zen_draw_input_field('subject', (isset($_POST['subject']) ? $_POST['subject'] : EMAIL_TEXT_SUBJECT), 'size="60"'); ?>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><?php echo zen_draw_textarea_field('message', 'soft', '100%', '15', ($_POST['message'] ?? ''), 'class="form-control"'); ?>
                        </td>
                    </tr>
                    <tr>
                        <!-- <td colspan="2" align="right"><?php /*echo ' class="btn btn-primary" role="button">' . IMAGE_RECOVER_CART_PREVIEW . '</a>';     */ ?></td> //-->
                        <td colspan="2" align="right" <?php echo '<a href="#" class="btn btn-primary" role="button">' . IMAGE_RECOVER_CART_SEND_EMAIL . '</a>'; ?></td>
                    </tr>
                </table>
            </td>
        </tr>
